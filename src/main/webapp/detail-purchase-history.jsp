							...
							<c:if test="${not empty sessionScope.user }">
								<li class="nav-item dropdown"><a id="navbarDropdown"
									...
									<div class="dropdown-menu dropdown-menu-end"
										...
										<a class="dropdown-item" href="/ShopShoes/users/information">User
											information</a> <a class="dropdown-item" href="/ShopShoes/cart">Cart</a>
										<a class="dropdown-item" href="/ShopShoes/users/purchase">Purchase
											history</a> <a class="dropdown-item"
											href="/ShopShoes/users/password">Change password</a> <a
						...
						<div class="card-body">
							<div class="container">
								<div class="form-group mb-3">
									<label for="">ID:</label> <input style="text-transform: none;"
										type="text" class="form-control" placeholder="ID.."
										value="${order.getId() }" readonly name="id">
								</div>
								<div class="form-group mb-3">
									<label for="">Name:</label> <input
										style="text-transform: none;" type="text" class="form-control"
										placeholder="Name.." value="${order.getName() }" readonly
										name="name">
								</div>
								<div class="form-group mb-3">
									<label for="">Status:</label> <input
										style="text-transform: none;" type="text" class="form-control"
										placeholder="Status.." value="Has paid" readonly name="status">
								</div>
								<table class="table">
									<thead>
										<tr>
									...
									<tbody>
										<c:forEach var="item" items="${order.getOrderDetail()}"
											varStatus="loop">
											<tr>
												<th scope="row">${loop.index + 1 }</th>
												<td>${item.getProductId()}</td>
												<td>${item.getProductName() }</td>
												<td><img alt="" class="w-50"
													src="/ShopShoes/${item.getProductImage() }"></td>
												<td>${item.getProductPrice() }</td>
												<td>${item.getAmount() }</td>
												<td>${item.getAmount() * item.getProductPrice()  }</td>
											</tr>
											<h3 hidden>${ TotalMoney = TotalMoney + item.getAmount()*item.getProductPrice()}</h3>
										</c:forEach>
									</tbody>
								</table>
								<h4>Total: ${TotalMoney } VND</h4>
							</div>
						</div>
					</div>
				</div>
