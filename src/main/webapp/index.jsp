		...
		<div class="container">
			<div style="position: relative;">
				<form class="form-inline d-flex"
					style="gap: 0 10px; position: absolute; right: 0; top: -40px;">
					<div class="form-group ">
						<label for="" class="sr-only">Password</label> <input type="text"
							class="form-control" style="margin: 0;" placeholder="Search">
					</div>
						...
						<ul class="list-group" style="overflow: auto; height: 250px;">
							<c:forEach var="item" items="${brands}" varStatus="indx">
								<li class="list-group-item"><a
									href="/ShopShoes/home?brandId=${item.getId()}">${item.getName() }</a></li>
							</c:forEach>
						</ul>
					</div>
					<div class="col-md-12 mb-2">
						<h3>Design:</h3>
						<ul class="list-group" style="overflow: auto; height: 250px;">
							<c:forEach var="item" items="${designs}" varStatus="indx">
								<li class="list-group-item"><a
									href="/ShopShoes/home?designId=${item.getId()}">${item.getName() }</a></li>
							</c:forEach>
						</ul>
					</div>
					<div class="col-md-12 mb-2">
						<h3>Brand:</h3>
						<ul class="list-group" style="overflow: auto; height: 250px;">
							<c:forEach var="item" items="${origins}" varStatus="indx">
								<li class="list-group-item"><a
									href="/ShopShoes/home?originId=${item.getId()}">${item.getName() }</a></li>
							</c:forEach>
						</ul>
					</div>
				</div>
					...
					<div class="container">
						<div class="row">
							...
							<c:if test="${empty products }">
								<h4 class="ms-4 text-danger">Product Empty...</h4>
							</c:if>
								...
								<div class="col-sm-6 col-md-4 col-lg-4">
									<div class="box">
										<div class="option_container">
											<div class="options">
												<a
													href="/ShopShoes/products/detail?productId=${item.getId() }"
													class="option1">Details</a> <a href="/ShopShoes/cart/add?productId=${item.getId() }"
										...
										<div class="detail-box"
											style="text-overflow: clip; gap: 0 10px; display: block !important; text-align: center ;">
											<h5 style="height: 100px;">${item.getName() }
											</h5>
						...
						<ul class="pagination justify-content-end">
							...
							<c:forEach begin="1" end="${ePage }" var="i">
								<li class="page-item ${tag ==i ? "active" : "" }"><a
									class="page-link" href='<c:url value="?indexP=${i }"/>'>${i }</a></li>
							</c:forEach>
